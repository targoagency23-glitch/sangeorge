...
export const AdminPortal: React.FC<AdminPortalProps> = ({ config, lang, onSave, onReset, onExit }) => {
  ...
  const handleFormSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(lang === "ar" ? "تم حفظ الإعدادات والصور بنجاح!" : "Configuration and media saved successfully!");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 bg-blue-50/20 p-5 rounded-xl border border-blue-100/60 grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
                <div>
                  ...
                  <MediaPickerField labelEn="Company Rectangular Logo" labelAr="شعار الشركة المستطيل المعتمد" currentValue={localConfig.logoUrl} onChange={(url) => setLocalConfig(prev => ({ ...prev, logoUrl: url }))} lang="ar" helperText="ارفع اللوجو الخاص بك هنا مباشرة. في حالة تركه فارغاً، سيتم دمج الـ 4 فروع وأشكال وإظهار مستطيل اللوجو المبتكر تلقائياً." uploadButtonLabelAr="اسحب أو ارفع اللوجو الآن" uploadButtonLabelEn="Upload Rectangular Logo" maxWidth={400} maxHeight={320} quality={0.75} />
                </div>
                <div>
                  <h4 className="text-xs font-Cairo font-extrabold text-[#172995] mb-2">📸 الصورة الرئيسية للشركة ومقر السويس</h4>
                  <MediaPickerField labelEn="Store / Warehouse Media Image" labelAr="الصورة الرئيسية لمخازن ومقرات الشركة" currentValue={localConfig.aboutMediaUrl} onChange={(url) => setLocalConfig(prev => ({ ...prev, aboutMediaUrl: url }))} lang="ar" helperText="Main visual displaying Suez warehouses, trading activities, or corporate showroom." uploadButtonLabelAr="رفع الصورة مباشرة" uploadButtonLabelEn="Upload Image Directly" maxWidth={800} maxHeight={600} quality={0.75} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-neutral-200/60 shadow-2xs space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <span className="text-3xs text-neutral-400 block">This passcode protects access to San George Suez dynamic configurations.</span>
              </div>
              <div className="space-y-1">
                ...
                <span className="text-3xs text-neutral-400 block text-right font-Cairo">رمز المرور المخصص لحماية وتعديل بيانات المعارض.</span>
              </div>
            </div>
          </div>
